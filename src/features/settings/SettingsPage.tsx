import { Tabs, Form, Input, InputNumber, DatePicker, Select, Switch, Button, Space, Card, Slider, message, Modal, Upload, Statistic, Row, Col, Popconfirm, TimePicker, Rate, Alert } from 'antd'
...
export default function SettingsPage() {
  const { examDate, setExamDate, targetSchool, setTargetSchool, targetScore, setTargetScore, theme, setTheme, dailyReminder, setDailyReminder, subjectProficiency, setSubjectProficiency, profile, setProfile } = useAppStore()
  ...
  useEffect(() => {
    basicForm.setFieldsValue({ examDate: examDate ? dayjs(examDate) : null, targetSchool, targetScore, theme, reminderEnabled: dailyReminder.enabled, reminderTime: dayjs(dailyReminder.time, 'HH:mm') })
  ...
  function handleBasicChange() {
    ...
    setTargetSchool(values.targetSchool || '')
    setTargetScore(values.targetScore || 120)
    ...
    if (reminderEnabled) {
      requestNotificationPermission().then((granted) => {
        if (!granted) {
          message.warning('通知权限未授予，每日提醒功能可能无法正常工作')
        }
      })
    }
  }
  ...
  async function handleImportQuestions() {
    try {
      ...
      const validQuestions: Question[] = questions.map((q: any) => ({
        subject: q.subject || importSubject,
        chapter: q.chapter || '',
        type: q.type || 'choice',
      ...
      message.error('导入失败：' + (e.message || 'JSON解析错误'))
    }
  }
  ...
  async function handleTestConnection() {
    ...
    setTesting(false)
    if (result.success) {
      message.success(`连接成功！延迟${result.latency}ms，响应: ${result.message}`)
    } else {
  ...
  async function handleExport() {
    ...
    useAppStore.getState().setLastBackupDate(dayjs().format('YYYY-MM-DD'))
  }
  ...
  async function handleClearAll() {
    await clearAllData()
    message.success('数据已清除，刷新页面生效')
    setTimeout(() => window.location.reload(), 1000)
  }

  return (
    <Card title="设置" styles={{ body: { padding: 0 } }}>
      <Tabs style={{ padding: '0 24px' }}
        items={[
            ...
            children: (
              <div style={{ padding: '20px 0', maxWidth: 720, margin: '0 auto' }}>
                <Form form={profileForm} layout="vertical" onValuesChange={handleProfileChange}>
                  <h3 style={{ marginBottom: 16, fontSize: 16 }}>基本信息</h3>
                  <Row>
                    <Col xs={24} sm={12} style={{ paddingRight: 8 }}>
                      <Form.Item label="姓名/昵称" name="name">
                        <Input prefix={<UserOutlined />} placeholder="你的名字" />
                      </Form.Item>
                    </Col>
                    <Col xs={24} sm={12} style={{ paddingLeft: 8 }}>
                      <Form.Item label="本科院校" name="undergradSchool">
                        <Input placeholder="如：华中科技大学" />
                      </Form.Item>
                    </Col>
                  </Row>
                  <Row>
                    <Col xs={24} sm={12} style={{ paddingRight: 8 }}>
                      <Form.Item label="本科专业" name="undergradMajor">
                        <Input placeholder="如：计算机科学与技术" />
                      </Form.Item>
                    </Col>
                    <Col xs={24} sm={12} style={{ paddingLeft: 8 }}>
                      <Form.Item label="每日学习时间（小时）" name="dailyStudyHours">
                        <InputNumber min={1} max={16} style={{ width: '100%' }} />
                      </Form.Item>
                    </Col>
                  </Row>

                  <h3 style={{ margin: '24px 0 16px', fontSize: 16 }}>科目自评（1-5星）</h3>
                  ...
                  <h3 style={{ margin: '24px 0 16px', fontSize: 16 }}>学习备注</h3>
                  <Form.Item label="我的薄弱点" name="weakPoints">
                    <Input.TextArea rows={3} placeholder="如：数据结构的图论部分、计组的流水线计算..." />
                  </Form.Item>
                  <Form.Item label="个人备注" name="notes">
            ...
            children: (
              <div style={{ padding: '20px 0', maxWidth: 720, margin: '0 auto' }}>
                <Form form={basicForm} layout="vertical" onValuesChange={handleBasicChange}>
                  ...
                  <Form.Item label="目标分数" name="targetScore">
                    <InputNumber min={0} max={150} style={{ width: '100%' }} />
                  </Form.Item>
                  <Form.Item label="主题" name="theme">
                    <Select options={[{ value: 'light', label: '亮色' }, { value: 'dark', label: '暗色' }]} />
                  </Form.Item>
                  <Form.Item label="每日提醒" name="reminderEnabled" valuePropName="checked">
                    <Switch />
                  </Form.Item>
            ...
            children: (
              <div style={{ padding: '20px 0', maxWidth: 720, margin: '0 auto' }}>
                <div style={{ marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {providers.map((p) => (
                    <Button key={p.id} type={p.id === activeProviderId ? 'primary' : 'default'} onClick={() => { setActiveProvider(p.id); setEditingProvider(p) }}>
                      {p.name}
                    </Button>
                  ))}
                  <Button icon={<PlusOutlined />} onClick={() => {
                    const np = { name: '新配置', providerType: 'openai' as const, apiEndpoint: '', apiKey: '', model: '', temperature: 0.7, maxTokens: 4096, systemPrompt: '' }
                    addProvider(np)
                  }}>新增</Button>
                </div>
                ...
                {providers.find((p) => p.id === activeProviderId) && (
                  <Form
                    ...
                    layout="vertical"
                    initialValues={providers.find((p) => p.id === activeProviderId)}
                    onValuesChange={(_, all) => {
                      updateProvider(activeProviderId, all)
                      setEditingProvider({ ...providers.find((p) => p.id === activeProviderId)!, ...all })
                    }}
                    style={{ maxWidth: '100%' }}
                  >
                    <Form.Item label="预设名称" name="name"><Input /></Form.Item>
                    <Form.Item label="接口类型" name="providerType">
                      <Select options={[{ value: 'openai', label: 'OpenAI兼容' }, { value: 'anthropic', label: 'Anthropic' }, { value: 'custom', label: '自定义' }]} />
                    </Form.Item>
                    ...
                    <Space>
                      ...
                      <Button icon={<SaveOutlined />} onClick={() => {
                        ...
                        if (provider) {
                          updateProvider(activeProviderId, provider)
                          message.success('保存成功')
                        }
                      }}>保存</Button>
                      <Popconfirm title="确定删除此预设？" onConfirm={() => removeProvider(activeProviderId)}>
          ...
          },
          {
            ...
            children: (
              <div style={{ padding: '20px 0', maxWidth: 720, margin: '0 auto' }}>
                <h3 style={{ fontSize: 16, marginBottom: 16 }}>数据概览</h3>
                <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
                  {Object.entries(stats).map(([key, count]) => (
                    <Col key={key} xs={12} sm={8} md={6}>
                      <Card size="small"><Statistic title={TABLE_LABELS[key] || key} value={count} /></Card>
                    </Col>
                  ))}
                </Row>

                <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                  <Card title="导出备份" size="small">
                    <p>将所有数据导出为JSON文件，建议定期备份。</p>
                    <Button type="primary" icon={<DownloadOutlined />} onClick={handleExport}>导出全部数据</Button>
                  </Card>

                  <Card title="导入备份" size="small">
                    <p>从JSON备份文件恢复数据（将覆盖现有数据）。</p>
                    <Upload accept=".json" beforeUpload={(file) => {
                      Modal.confirm({
                        title: '确认导入备份？',
                        content: '此操作将覆盖当前所有数据和设置（包括AI配置、个人设置等），不可恢复！',
                        okText: '确认导入',
                        okType: 'danger',
                        cancelText: '取消',
                  ...
                  <Card title="自定义题目导入" size="small">
                    <p>从JSON文件或文本框导入自定义题目到题库。支持批量导入。</p>
                    <Upload accept=".json" beforeUpload={async (file) => {
                  ...
                  <Card title="清除数据" size="small" style={{ borderColor: '#ff4d4f' }}>
                    <p style={{ color: '#ff4d4f' }}>此操作不可恢复，请确保已备份。</p>
  ...
  "tags": ["标签1","标签2"]
        ...
        <Select
          ...
          options={Object.entries(SUBJECT_NAMES).map(([k, v]) => ({ value: k, label: v }))}
          placeholder="默认科目（可在JSON中覆盖）"
        />
        <Input.TextArea
          rows={10}
          value={importText}
          onChange={(e) => setImportText(e.target.value)}
          placeholder="粘贴JSON格式的题目数组..."
